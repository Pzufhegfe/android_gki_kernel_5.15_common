...
if [ "$SKIP_BUILD" = false ]; then
    ...
    # 确保进入源码目录获取时间
    pushd "$WORKSPACE_DIR/common" > /dev/null
        # 获取最新 commit 的时间戳（稳定值，不会导致缓存失效）
        COMMIT_TIME=$(git log -1 --format=%ct 2>/dev/null || echo "0")
        # 将时间戳格式化为字符串（使用固定格式确保一致性）
        if command -v date >/dev/null 2>&1; then
            # 尝试使用 GNU date (Linux)
            BUILD_TIMESTAMP=$(date -u -d "@$COMMIT_TIME" "+%Y-%m-%d %H:%M:%S %Z" 2>/dev/null || \
                             date -u -r "$COMMIT_TIME" "+%Y-%m-%d %H:%M:%S %Z" 2>/dev/null || \
    ...
    # 使用 commit 时间戳确保可重现构建
    # 注意：这些环境变量会作为缓存键的一部分，所以必须稳定
    export SOURCE_DATE_EPOCH=$COMMIT_TIME
    export KBUILD_BUILD_TIMESTAMP="$BUILD_TIMESTAMP"

    log "Build timestamp pinned to commit time: $BUILD_TIMESTAMP (epoch: $COMMIT_TIME)"
    
    # 🔥 CRITICAL FIX: 使用稳定的时间戳确保缓存有效
    # 使用 --action_env 传递时间戳，但值基于 commit 时间（稳定）
    # 这样即使有未提交的更改，只要 commit 相同，时间戳就相同，缓存就能命中
    # Bazel 缓存基于文件内容哈希，不依赖 git 状态，所以未提交的更改不会影响缓存
    # 只要文件内容相同，即使有未提交的更改，缓存也能正常工作
    ...
    log "Cache optimization: Ignoring uncommitted changes, using stable commit timestamp"
    log "Cache key factors: COMMIT_TIME=$COMMIT_TIME, BUILD_TIMESTAMP=$BUILD_TIMESTAMP"
